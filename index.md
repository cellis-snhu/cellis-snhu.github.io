...
### Description
For the artifact entitled Databases, I had planned to integrate changes made from my enhancement for Software Engineering and Design, by taking an artifact and developing a simple piece of code into a full stack application including a separate standalone database, along with a data model and data access layer code in order for the full stack application to use it.

In planning for this, I decided on using a SQLite database for my development and testing, with the idea that I would also include a separate configuration to use a full fledged RDBMS, in this case Postgresql Server for the final portfolio submission. In reality SQLite will always be used as the development environment as it works like any SQL database, while a separate config line will allow non-dev environments to use a Postgresql database. This is achieved by using an underlying Python library, SQLAlchemy, to abstract away the specifics of the database used into generic SQL and allow the use of Python objects based on the SQLAlchemy classes like Db.Model to map code to database queries and schema.

The data model is based on a standard User model used in nearly every web application that has users, with a User object including fields like username, password hash, email, etc. The object model developed in enhancement 1 for the Task objects in a task manager will be modified from standard Python classes to use Db.Model as their base and thus make them usable in the database. The goal to achieve for this portion of the enhancement was the development of the User model and code related to registration, sign-in, sign out and standard user functionality along with mapping it to the database, as a proof of concept of the working database. Additionally, setup of a docker container based Postgresql server and connection of the code to that is an important step I plan to finish for this milestone as that will be the database used in the final product. Time permitting for this milestone due to the complexity and additional features needed, I would also like to convert the Task objects to Db.Model as well as tie them into the User code with foreign keys between them to map tasks to a user in the database, however this is only needed for the final product.

### Justification
The database enhancement requirements themselves were not specific on the scale or scope of what was needed for the project to meet the course outcomes specifically, so I chose a project idea that was both comprehensive in scope to database operations themselves, including the setup and configuration of a database designing a data model and SQL queries, as well as narrow in scope to integrating that database specifically with a programming language in a useful application. In my opinion this covers the possible requirements exceptionally well based on the course outcomes that it helped me work towards.
...
The use of both well founded and innovative techniques was covered in the standard setup and configuration of a database along with the standard style of User model code used by most web applications, as well as the specific innovative techniques included in mapping the existing task management code to the database along with developing a coherent data model for it.

The security mindset is explicitly performed as well, and is especially needed when working with databases as they are often a single point that stores a large amount of user specific data such as passwords (hopefully only hashes and not text), as well as people’s email, address, billing information, and personal info related to the data used by that application. In doing this I followed standard security procedures including writing tests and validators to ensure security against things like SQL injection attacks that may leak database information, as well as using frameworks and libraries for hashing passwords and procedures to secure user credentials in a database.

### Enhancement Process

In building this enhancement it was required that I decouple the initial software engineering work from the database development, for which I just developed an independent model based on Python classes with tests knowing that I would need to refactor that for the final iteration of the database enhancement. Beginning the database enhancement itself, I started by setting up a SQLite database, and creating standard User models using the SQLAlchemy Db.Model class which is often used in Python web app frameworks like Python, Django, FastAPI and others to auto generate schemas based on python objects. After this I tested initializing a database and applying the schema, along with creating User objects on the command line from the flask shell using my new db models. After this I began development of the login and registration code, adding those pages and routes to the existing application. At this time I have not completed the refactor of the Task objects to Db.Model objects as that is only required for the final product to be complete, but everything else is in place for that refactor to happen. I also included a docker-compose.yml file that setups a local Postgresql server based database for testing against the real RDBMS that would be run with the application when it is hosted. I have performed this operation many times before so much of it was similar to projects I have done in the past. A new thing I have implemented was having many of the tests written before the refactor which I believe will help greatly in the overall change to Db.Model for tasks as having the tests allowed rapid development and change for all of the other pieces of the application that rely on the Task object (the initial code it was ported from was in Java but had tests, so I converted the tests first in a test driven development manner and then developed the Python code, using the tests to prove that my conversions were correct and worked with the expected validations I had included in my code.
